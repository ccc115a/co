...
此資料夾 (`_web_eda16/lib/hdl/`) 包含了在瀏覽器（Web EDA）上運行硬體描述語言 (HDL) 模擬器的核心編譯工具鏈。這套系統將 Nand2Tetris 課程中的 HDL 檔案轉換為可以用極高效率在網路上執行的 JavaScript / WebAssembly 程式碼。
...
32-bit 版本即 `../_web_eda/`（姊妹引擎），它把 `sub()/setBits()` 的 mask 改成
`0xffffffff`（JS 位元運算對 Number 仍然安全）、`Uint16Array` 換成
`Uint32Array`，並自行提供了 32-bit 版本的 Register/RAM/ROM 內建與原生融合規則。
